...
class VTgtField(object):
    ...
    ver["ver01"]["v_amp_rng"] = np.array([0.8, 1.8])
    ver["ver01"]["rng_p_sink_r_th"] = np.array([[5, 7], [0, 0]])
    ver["ver01"]["r_target"] = 0.3
    ...
    ver["ver02"]["v_amp_rng"] = np.array([0.8, 1.8])
    ver["ver02"]["rng_p_sink_r_th"] = np.array(
        [[5, 7], [-90 * np.pi / 180, 90 * np.pi / 180]]
    )
    ver["ver02"]["r_target"] = 0.3
    ...
    ver["ver03"]["v_amp_rng"] = np.array([0.8, 1.8])
    ver["ver03"]["rng_p_sink_r_th"] = np.array(
        [[5, 7], [-180 * np.pi / 180, 180 * np.pi / 180]]
    )
    ver["ver03"]["r_target"] = 0.3
    ...
    # -----------------------------------------------------------------------------------------------------------------
    def __init__(
        self, visualize=False, version=1, dt=0.01, dt_visualize=0.5, seed=None
    ):
        self.dt = dt
    ...
    def reset(self, version=1, seed=None, pose_agent=np.array([0, 0, 0])):
        ...
        self.res_get = np.array(
            [
                (self.rng_get[0, 1] - self.rng_get[0, 0] + 1) / self.nn_get[0],
        ...
        if self.ver["version"] == 0:
            ...
            self.vtgt_obj = VTgtConst(
                v_tgt=self.ver["ver00"]["v_amp"],
                rng_xy=self.rng_xy,
                res_map=self.res_map,
                rng_get=self.rng_get,
                res_get=self.res_get,
            )
            self.create_vtgt_const(v_tgt=self.ver["ver00"]["v_amp"])
        elif self.ver["version"] in [1, 2, 3]:
            # map coordinate and vtgt
            self.vtgt_obj = VTgtSink(
                rng_xy=self.rng_xy,
                res_map=self.res_map,
                rng_get=self.rng_get,
                res_get=self.res_get,
            ...
            # create first sink
            del_p_sink_r = np.random.uniform(
                self.rng_p_sink_r_th[0, 0], self.rng_p_sink_r_th[0, 1]
            )
            del_p_sink_th = np.random.uniform(
                self.rng_p_sink_r_th[1, 0], self.rng_p_sink_r_th[1, 1]
            )
        ...
        if self.visualize:
            ...
            self.vis = {}
            self.vis["plt"] = plt
            self.vis["hndl"], self.vis["axes"] = self.vis["plt"].subplots(
                2, 1, figsize=(5, 6)
            )
            ...
            X, Y = self.vtgt_obj._generate_grid(
                self.vtgt_obj.rng_get, self.vtgt_obj.res_get
            )
            U = v_tgt_field[0]
            V = v_tgt_field[1]
            R = np.sqrt(U**2 + V**2)
    ...
    def create_vtgt_sink(self, v_amp_rng):
        ...
        self.vtgt_obj.create_vtgt_sink(
            self.p_sink, d_sink, v_amp_rng, v_phase0=v_phase0
        )
    ...
    def update(self, pose):
        ...
        if not hasattr(self, "p_sink"):
            ...
            flag_target_achieved = 0
            if (
                self.t_target > self.t0_target  # stayed at the target
                and self.i_target <= self.n_new_target
            ):  # on a new target
...
class VTgt0(object):
    ...
    # -----------------------------------------------------------------------------------------------------------------
    def _generate_grid(
        self, rng_xy=np.array([[-10, 10], [-10, 10]]), res=np.array([2, 2])
    ):
        xo = 0.5 * (rng_xy[0, 0] + rng_xy[0, 1])
        x_del = (rng_xy[0, 1] - xo) * res[0]
        yo = 0.5 * (rng_xy[1, 0] + rng_xy[1, 1])
    ...
    def get_vtgt_field_local(self, pose):
        ...
        get_map_x = (
            np.cos(th) * get_map0[0, :, :] - np.sin(th) * get_map0[1, :, :] + xy[0]
        )
        get_map_y = (
            np.sin(th) * get_map0[0, :, :] + np.cos(th) * get_map0[1, :, :] + xy[1]
        )

        # get vtgt
        points = np.stack(
            [get_map_x.flatten(), get_map_y.flatten()], axis=-1
        )  # shape (N,2)
...
class VTgtSink(VTgt0):
    ...
    def __init__(
        ...
        super(VTgtSink, self).__init__(
            rng_xy=rng_xy, res_map=res_map, rng_get=rng_get, res_get=res_get
        )
        self.vtgt = -1 * self.map

    # -----------------------------------------------------------------------------------------------------------------
    def create_vtgt_sink(
        self, p_sink, d_sink, v_amp_rng, v_phase0=np.random.uniform(-np.pi, np.pi)
    ):
        ...
        self.vtgt_interp_x = RegularGridInterpolator(
            (x, y), self.vtgt[0].T, bounds_error=False, fill_value=None
        )
        self.vtgt_interp_y = RegularGridInterpolator(
            (x, y), self.vtgt[1].T, bounds_error=False, fill_value=None
        )
        ...
        for i_x, x in enumerate(self.map[0, :, 0]):
            for i_y, y in enumerate(self.map[1, 0, :]):
                ...
                if d > d_sink + d_dec:
                    ...
                    v_amp = 0.5 * np.diff(v_amp_rng) * np.sin(v_phase) + np.mean(
                        v_amp_rng
                    )
                else:
                    v_phase = v_phase0 + d_dec / d_sink * 2 * np.pi
                    v_amp0 = 0.5 * np.diff(v_amp_rng) * np.sin(v_phase) + np.mean(
                        v_amp_rng
                    )
                    v_amp = d * v_amp0
